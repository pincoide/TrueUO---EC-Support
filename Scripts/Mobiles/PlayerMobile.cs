		...
		*/
        ...
        public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
        {
            ...
            if ( from == this )
            {
                ...
                if ( house != null )
                {
                    ...
                    {
                        list.Add( new CallbackEntry( 6205, ReleaseCoOwnership, 602 ) );
                    }
                }
                ...
                {
                    list.Add( new CallbackEntry( 1114299, OpenItemInsuranceMenu, 416 ) );
                    list.Add( new CallbackEntry( 6201, ToggleItemInsurance, 418 ) );
                }
                else if ( Siege.SiegeShard )
                {
                    list.Add( new CallbackEntry( 6168, SiegeBlessItem, 820 ) );
                }
                ...
                if ( house != null )
                {
                    ...
                    {
                        list.Add( new CallbackEntry( 6204, GetVendor, 601 ) );
                    }

                    list.Add( new CallbackEntry( 6207, LeaveHouse, 604 ) );
                }

                list.Add( new CallbackEntry( RefuseTrades ? 1154112 : 1154113, ToggleTrades, RefuseTrades ? 1013 : 1014 ) ); // Allow Trades / Refuse Trades				

                if ( m_JusticeProtectors.Count > 0 )
                {
                    list.Add( new CallbackEntry( 6157, CancelProtection, 308 ) );
                }
                ...
                {
                    list.Add( new CallbackEntry( 1113797, EnablePvpWarning, 320 ) );
                }
            }
            else
            {
                ...
                {
                    list.Add( new CallbackEntry( 1077728, () => OpenTrade( from ), 819 ) ); // Trade
                }
        ...
        private class ItemInsuranceMenuGump : Gump
        {
                ...
                : base( 25, 50 )
            {
                ...
                AddLabel( 215, 420, 0x481, balance.ToString( "N0", new CultureInfo( "en-US" ) ) );
                AddHtmlLocalized( 15, 435, 300, 20, 1114123, 0x7FFF, false, false ); // TOTAL COST OF INSURANCE:
                AddLabel( 215, 435, 0x481, cost.ToString( "N0", new CultureInfo( "en-US" ) ) );

                AddHtmlLocalized( 15, 450, 300, 20, 1114125, 0x7FFF, false, false ); // NUMBER OF DEATHS PAYABLE:

                if ( cost != 0 )
                    AddLabel( 215, 450, 0x481, ( balance / cost ).ToString( "N0", new CultureInfo( "en-US" ) ) );
                else
                    AddLabel( 215, 450, 0x481, "0" );
        ...
        private class CallbackEntry : ContextMenuEntry
        {
            ...
            public CallbackEntry( int number, ContextCallback callback, int returnCode = -1 )
                : this( number, -1, callback, returnCode )
            { }

            public CallbackEntry( int number, int range, ContextCallback callback, int returnCode = -1 )
